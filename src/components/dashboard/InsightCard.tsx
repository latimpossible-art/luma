...
export function InsightCard({ insight, onRefresh, isVisible = true, onHide, className }: InsightCardProps) {
    ...
    return (
        <motion.div
            ...
            className={`bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-border dark:border-slate-800 relative transition-colors ${className}`}
        >
            <div className="flex items-center justify-between mb-3 relative">
                ...
                <div className="relative">
                    <button
                        ...
                        className="text-muted-foreground hover:text-foreground p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    >
                    ...
                    <AnimatePresence>
                        {showMenu && (
                            <motion.div
                                ...
                                className="absolute right-0 top-8 z-20 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden"
                            >
                                <button
